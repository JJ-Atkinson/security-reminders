            ...
            (when link
              [:button
               {:class (into ["text-sm" "whitespace-nowrap"] ui.frag/button-classes)
                :_     (str "on click writeText('" link
                            "') into navigator.clipboard"
  ...
  (let [params (:params request)
    ...
    (cond
      ...
      (let [env   (time-layer/scheduler-env time-layer)
            id    (str "p-" (System/currentTimeMillis))
            token (engine/generate-token)]
        (engine/with-state!-> env
          (ops/add-person {:id id :name (str/trim pname) :email email :admin? admin?})
