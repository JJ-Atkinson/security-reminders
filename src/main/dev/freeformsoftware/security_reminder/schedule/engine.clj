(ns dev.freeformsoftware.security-reminder.schedule.engine
  "Impure shell for schedule state management.
   Owns disk I/O, SMS sending, and token generation.
...
(>defn generate-token
  "Generate a short random base46 token (6 chars).
...
(>defn resolve-state!
  "Process accumulated actions (SMS sending, token rotation) and write final state to disk.
   Returns nil."
  [{:keys [db-folder twilio-conf base-url on-assignment-change] :as _env} state]
  [map? map? => any?]
  (let [actions (:actions state)
        people-by-id (into {} (map (juxt :id identity)) (get-in state [:schedule-db :people]))
        ;; Process actions, accumulating token updates into state
        final-state
        (reduce
         (fn [state action]
           (try
...
(defmacro with-state!->
  "Fetch state, thread through pure ops, resolve (SMS + write).
