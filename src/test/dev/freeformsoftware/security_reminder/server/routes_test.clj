   ...
   (fn [eng]
     (let [conf    (make-conf eng)
       ...
       (testing "invalid email returns 400"
         (let [resp (handler (admin-request token {:name "Test" :email "not-an-email"}))]
           (is (= 400 (:status resp)))))

       (testing "email without @ returns 400"
         (let [resp (handler (admin-request token {:name "Test" :email "testexample.com"}))]
           (is (= 400 (:status resp)))))

       (testing "valid input returns redirect"
         (let [resp (handler (admin-request token {:name "Test" :email "test@example.com"}))]
           (is (= 302 (:status resp)))))))))
