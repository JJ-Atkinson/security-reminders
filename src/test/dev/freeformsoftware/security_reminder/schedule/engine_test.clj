     ...
     (fn [eng]
       (let [plan        (engine/view-plan eng)
         ...
         (engine/with-state!-> eng
           (ops/note-absence person-id event-key))
    ...
    (h/with-test-engine
     (fn [eng]
       (let [plan-before          (engine/view-plan eng)
         ...
         (engine/with-state!-> eng
           (ops/set-instance-override event-date template-id 4))
         (let [plan-after (engine/view-plan eng)
    ...
    (h/with-test-engine
     (fn [eng]
       (let [plan                 (engine/view-plan eng)
             ...
             template-id          (get-in first-template-event [:event-key :template-id])]
         (engine/with-state!-> eng
           (ops/set-instance-override event-date template-id 3))
         (engine/with-state!-> eng
           (ops/set-instance-override event-date template-id 5))
    ...
    (h/with-test-engine
     (fn [eng]
       (let [event-key {:date "2026-04-01" :template-id "et-1"}]
         ...
         (engine/with-state!-> eng
           (ops/record-notification-sent "p1" event-key :reminder (engine/now-str)))
         ...
         (engine/with-state!-> eng
           (ops/record-notification-sent "p2" event-key :assigned (engine/now-str)))
         (is (not (engine/reminder-already-sent? eng "p2" event-key))))))))
    ...
    (h/with-test-engine
     (fn [eng]
       (let [ek1 {:date "2026-04-01" :template-id "et-1"}
         ...
         (engine/with-state!-> eng
           (ops/record-notification-sent "p1" ek1 :reminder (engine/now-str)))
         (Thread/sleep 10)
         (engine/with-state!-> eng
           (ops/record-notification-sent "p2" ek2 :assigned (engine/now-str)))
    ...
    (h/with-test-engine
     (fn [eng]
       (let [ek {:date "2026-04-01" :template-id "et-1"}]
         (dotimes [i 510]
           (engine/with-state!-> eng
             (ops/record-notification-sent (str "p" i) ek :reminder (engine/now-str))))
         (let [db (engine/view-db eng)]
    ...
    (h/with-test-engine
     (fn [eng]
       (let [plan      (engine/view-plan eng)
         ...
         (engine/with-state!-> eng
           (ops/set-assignment-override event-key ["p4" "p5"]))
         (let [new-plan (engine/view-plan eng)
    ...
    (h/with-test-engine
     (fn [eng]
       (let [plan      (engine/view-plan eng)
             ...
             event-key (:event-key target)]
         (engine/with-state!-> eng
           (ops/set-assignment-override event-key ["p1" "p2"]))
         (engine/with-state!-> eng
           (ops/set-assignment-override event-key ["p3" "p4"]))
         (let [db        (engine/view-db eng)
