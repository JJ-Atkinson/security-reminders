...
(deftest test-add-one-off
  (testing "add-one-off! event appears in view-plan"
    (h/with-test-engine
      (fn [eng]
        (engine/add-one-off! eng {:label "Special Event"
                                  :date "2026-04-15"
...
(deftest test-remove-one-off
  (testing "remove-one-off! event disappears from plan"
    (h/with-test-engine
      (fn [eng]
        (engine/add-one-off! eng {:label "Temp Event"
                                  :date "2026-04-15"
                                  :time-label :afternoon
                                  :people-required 2})
...
(deftest test-plan-persistence
  (testing "plan survives engine restart"
    (let [tmp-dir (str (fs/create-temp-dir {:prefix "sr-persist-"}))]
      (try
        (let [eng1 (ig/init-key ::engine/engine {:db-folder tmp-dir :people h/test-people
                                                  :twilio-conf h/test-twilio-conf
                                                  :base-url "http://test.local"})
              env1 (assoc eng1 :on-assignment-change (fn []))]
          (engine/add-one-off! env1 {:label "Persist Test"
                                     :date "2026-04-15"
                                     :time-label :afternoon
                                     :people-required 2})
          (ig/halt-key! ::engine/engine eng1))
        ;; Restart with same dir
        (let [eng2 (ig/init-key ::engine/engine {:db-folder tmp-dir :people h/test-people
                                                  :twilio-conf h/test-twilio-conf
                                                  :base-url "http://test.local"})
    ...
    (let [tmp-dir (str (fs/create-temp-dir {:prefix "sr-migrate-"}))]
      (try
        ...
        (let [db-file (str (fs/path tmp-dir "schedule-db.edn"))]
          (spit db-file (pr-str {:event-templates [{:id "et-1" :label "Wed Evening"
                                                    :day-of-week 3 :time-label :evening
                                                    :people-required 2}]
                                 :one-off-events []
                                 :absences []
                                 :sent-reminders [{:person-id "p1" :event-date "2026-04-01"
                                                   :sent-at "2026-03-25T10:00:00"}]
                                 :sent-notifications []
                                 :instance-overrides []
                                 :sec-tokens {}
                                 :people h/test-people}))
          (let [eng (ig/init-key ::engine/engine {:db-folder tmp-dir :people h/test-people
                                                    :twilio-conf h/test-twilio-conf
                                                    :base-url "http://test.local"})
